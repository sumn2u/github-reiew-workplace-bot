...
if (config.github) {
    ...
    options.debug = true;
    options.Promise= require('bluebird');
...
function getPullRequests(callback) {
    ...
    github.pullRequests.getAll({
        user: config.user,
        repo: config.repo,
        state: config.pullRequestsStatus,
        owner: config.owner
    }, function (error, result) {
        if (error) {
            return debug('getPullRequests: Error while fetching PRs: ', error);
        }
        if (!result || !result.data.length || result.data.length < 1) {
     ...
     */
     ...
     github.issues.getComments({
        ...
        for (var i = 0; i < result.data.length; i++) {
            if (result.data[i].body && lgtm.test(result.data[i].body)) {
                // Test if we're actually just in the instructions comment
                isInstruction = (result.data[i].body.slice(0, 30).trim() === config.instructionsComment.slice(1, 30).trim());
...
function checkForInstructionsComment(prNumber, callback) {
    ...
    github.issues.getComments({
        ...
        for (var i = 0; i < result.data.length; i++) {
            instructed = (result.data[i].body.slice(1, 30).trim() === config.instructionsComment.slice(1, 30).trim());
            if (instructed) {
                break;
            }
        }
    ...
    github.pullRequests.getFiles({
        ...
        for (i = 0; i < result.data.length; i = i + 1) {
            for (var ii = 0; ii < filenameFilter.length; ii = ii + 1) {
                ...
                match = (result.data[i].filename.indexOf(filenameFilter[ii]) > -1) ? true : match;

                if (match) {
                    return callback(result.data[i]);
                }
            }
        }
...
function checkForStatus(sha, callback) {
    ...
    github.repos.getCombinedStatusForRef({
        ...
        return callback(result.data.state == 'pending')
    })
}
